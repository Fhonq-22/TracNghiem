...
export async function themCauHoi(cauHoi) {
    await addData("CauHoi", cauHoi.MaCauHoi, cauHoi.toJSON());
}

export async function layCauHoi(maCauHoi) {
    const data = await getData("CauHoi", maCauHoi);
    return data ? new CauHoi(maCauHoi, data.NoiDung, data.PhuongAn, data.DapAnDung) : null;
}
...
export async function suaCauHoi(maCauHoi, cauHoiMoi) {
    await updateData("CauHoi", maCauHoi, cauHoiMoi.toJSON());
}
